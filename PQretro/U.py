...
#                  x.minInQ)
    ...
    # ENCONTRA O ULTIMO NODE INSERT MENOR OU IGUAL AO NODE X
    # EVITAMOS O BUG SE A PONTE DO REMOVE (DELETE_delMin no meu caso)
    # SEMPRE FOR UMA OPERAÇÂO DE Insert
    def LastInsertBefore(self, x):
        if x.maxNotQ[0] != -inf:
            return x

        y = x.p
        while x is not self.root:
            if y.right is x and y.left.maxNotQ[0] != -inf:
                x = y.left
                break
            x = x.p
        while x.data is None:
            if x.right.maxNotQ[0] != -inf:
                x = x.right
            else:
                x = x.left
        return x
    ...
    def LastBridgeBeforeFinder(self, t):
        ...
        flagEspinhaEsquerda = True # consertar um bug raro
...
#                    print(y.left.key)
    ...
    # a flagRem indica se estamos numa operacao de Delete_delMin
    # ou de Insert_insert, onde tratamos os casos diferentemente
    def FindBigKeyAfterBridge(self, t, flagRem):
...
#        if flagRem is True and maxNode[0] == -inf:# caso do bug
#            if self.root.right.maxNotQ[0] == -inf:
#                maxNode = self.root.left.maxNotQ
#            else:
#                maxNode = self.root.right.maxNotQ
        if flagRem is True and maxNode[0] == -inf:
            maxNode = self.LastInsertBefore(xBridge).maxNotQ
        return maxNode

    def BigKeySearchNotQ(self, maxNode):
        return self.Search(maxNode[1])[0]

    def KeyToInsert(self, t, flagRem):
        tupla = self.FindBigKeyAfterBridge(t, flagRem)
        ...
        while x is not self.root:
            ...
            if y.left is x:
                if partSum + y.right.minSum == 0:
                    x = y.right
                    flagEspinhaDireita = False
                    break
                else:
                    partSum += y.right.subSum
            x = x.p

        if flagEspinhaDireita is True:
            return self.TreeMax(self.root)
